...
const Captcha = ({ onVerify, className }: CaptchaProps) => {
  ...
  return (
    <div 
      ...
      className={cn(
        "flex items-center gap-3 p-2 px-3 rounded-lg border border-border/60 bg-background cursor-pointer select-none transition-all",
    ...
    >
      <div className="flex items-center justify-center size-5">
        ...
        {(status === 'loading' || status === 'computing') && <span className="icon-[ph--spinner-gap] size-5 text-primary animate-spin"></span>}
      ...
      <span className="text-xs font-medium text-muted-foreground">
        {status === 'idle' ? 'Verify you are human' : 
         status === 'loading' ? 'Connecting to edge...' :
         status === 'computing' ? 'Securing connection...' : // 增加一个计算中的文案
         status === 'success' ? 'Verification complete' : 'Verification failed, retry'}
      </span>
    </div>
  );
};

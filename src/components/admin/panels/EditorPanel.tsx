...
export default function EditorPanel() {
  ...
  return (
    <div className={cn(
        ...
        showLeftPanel && showRightPanel ? "lg:col-span-6 xl:col-span-7" : !showLeftPanel && showRightPanel ? "lg:col-span-9" : showLeftPanel && !showRightPanel ? "lg:col-span-9 xl:col-span-10" : "lg:col-span-12"
    )}>
      {/* 顶部工具栏 */}
      <div className="h-12 flex justify-between items-center border-b border-border/40 bg-muted/10 relative z-10 px-2 sm:px-4">
          <div className="flex items-center h-full flex-1 min-w-0">
               <button onClick={() => setShowLeftPanel(!showLeftPanel)} className="hidden lg:flex p-1.5 mr-2 rounded-xs text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
               ...
               <div className="flex-1 flex items-center gap-2 max-w-sm">
                    <span className={cn("size-4 text-muted-foreground", currentMode === 'data' ? "icon-[ph--brackets-curly]" : "icon-[ph--file-text]")}></span>
                    <input 
                      value={filename} 
          ...
          <div className="flex items-center gap-1 sm:gap-2">
              ...
              <button onClick={stageForWrite} className="px-4 py-1.5 rounded-xs bg-foreground text-background text-xs font-semibold hover:bg-foreground/90 transition-all flex items-center gap-1.5 shadow-xs">
                 <span className="icon-[ph--floppy-disk] size-4"></span> 
                 <span className="hidden sm:inline">Save</span>
              </button>

              <button onClick={() => setShowRightPanel(!showRightPanel)} className="hidden lg:flex p-1.5 ml-1 rounded-xs text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">

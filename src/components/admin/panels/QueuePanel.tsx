                ...
                <div key={item.id} className="relative bg-background border border-border/40 rounded-xs p-3 flex flex-col gap-2 group hover:border-border hover:shadow-xs transition-all">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-1">
                        {item.type === 'write' && (
                            <button onClick={() => loadFromQueue(item)} className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Edit">
                    ...
                    <div>
                      <div className="text-sm font-medium truncate text-foreground" title={item.filename}>{item.filename.split('/').pop()}</div>
                      {item.status !== 'pending' && (
                        <div className={cn(

    ...
    >
      <div className="h-12 px-4 border-b border-border/40 flex justify-between items-center bg-muted/20 shrink-0">
        ...
        <div className="flex gap-1">
          <button onClick={handleNewPost} className="px-2 py-0.5 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="New Post">
            <span className="icon-[ph--plus] size-3.5" />
          </button>
          <button onClick={() => fetchRemoteFiles()} className="px-2 py-0.5 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Refresh">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 min-h-0">
        ...
        <div className="space-y-0.5 mb-4">
          {DATA_FILES.map((f) => (
            <div key={f.name} onClick={() => loadFile(f.name, true, f.path)} className={cn('group flex items-center justify-between text-sm px-3 py-2 rounded-md cursor-pointer transition-all', filename === f.name ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-muted')}>
              <div className="flex items-center gap-2.5 overflow-hidden">
                <span className={cn('icon-[ph--brackets-curly] size-4 shrink-0', filename === f.name ? 'text-primary' : 'text-muted-foreground/60')} />
        ...
        <div className="px-2 py-2 text-[10px] font-mono text-muted-foreground/50 uppercase tracking-widest border-t border-border/40 mt-2 pt-4">Posts & Articles</div>
            ...
            sortedRemoteFiles.map((f) => (
              <div key={f.sha} className={cn('group flex justify-between items-center text-sm px-3 py-2 rounded-md cursor-pointer transition-all', filename === f.name ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-muted')}>
                ...
                <button
                  ...
                  className="text-muted-foreground/40 hover:text-red-500 p-1 rounded-md hover:bg-red-500/10 transition-colors shrink-0"
                  title="Delete"

...
export default function LoginScreen() {
  ...
  return (
    <div className="relative font-sans flex flex-col lg:flex-row min-h-screen max-w-[1920px] mx-auto text-foreground overflow-hidden">
      ...
      {/* MOBILE LEFT PANE */}
      <div className="lg:hidden relative w-full h-[30vh] bg-muted/10 border-b border-border p-8 flex flex-col justify-between overflow-hidden select-none">
        ...
        <div className="relative z-10 flex items-center gap-3 text-[10px] text-muted-foreground uppercase tracking-widest">
            <div className={cn("size-2 rounded-full", loginError ? "bg-red-500 animate-pulse" : "bg-primary")}></div><span>System Status: {loginError ? 'ALERT' : 'LOCKED'}</span>
        </div>
        <div className="relative z-10 mt-4">
            <h1 className="text-4xl font-black uppercase tracking-tighter leading-[0.9] opacity-90 mb-4">Identity<br/>Core_V3</h1>
            <div className="border-l-2 border-primary pl-4 py-1"><p className="text-xs text-muted-foreground uppercase tracking-[0.2em]">Restricted Access Area</p><p className="text-[10px] text-muted-foreground/60 uppercase mt-1">Auth_Protocol: SHA-256</p></div>
        </div>
      </div>

      {/* PC LEFT PANE */}
      <aside className="hidden lg:flex w-1/3 sticky top-0 h-screen flex-col border-r  z-20 bg-muted/5 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)] dark:shadow-none select-none">
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'linear-gradient(currentColor 1px, transparent 1px), linear-gradient(90deg, currentColor 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
        <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col relative z-10">
          <div className="p-8 xl:p-12 pb-0 shrink-0"><div className="flex items-center gap-3 text-[10px] text-muted-foreground uppercase tracking-widest font-mono"><div className={cn("size-2 rounded-none", loginError ? "bg-red-500 animate-pulse" : "bg-primary")}></div><span>System Status: {loginError ? 'ALERT' : 'LOCKED'}</span></div></div>
          <div className="flex-1 flex flex-col justify-center p-8 xl:p-12 min-h-[250px]">
              ...
              <div className="border-l-2 border-primary pl-4 py-1"><p className="text-sm text-muted-foreground uppercase tracking-[0.2em] font-mono">Restricted Access Area</p><p className="text-[10px] text-muted-foreground/60 uppercase mt-2 font-mono">Auth_Protocol: SHA-256</p></div>
          </div>
          <div className="shrink-0 p-8 xl:p-12 border-t  border-dashed"><div className="flex justify-between items-end text-[9px] text-muted-foreground/40 font-bold font-mono"><div className="flex flex-col gap-1"><span>SECURE_GATEWAY</span><span>NODE: 099-AX</span></div><div className="text-right">UNAUTHORIZED ACCESS<br/>WILL BE LOGGED</div></div></div>
        </div>
      </aside>

      {/* RIGHT PANE (Form Area) */}
      <main className="w-full lg:w-2/3 flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative bg-background">
         <div className="absolute top-4 right-4 md:top-8 md:right-8 text-[9px] font-bold text-muted-foreground border  px-2 py-1 opacity-50 font-mono select-none">TERMINAL_ID: T-800</div>
         <div className="w-full max-w-sm space-y-10 z-10">
            <div className="lg:hidden border-l-2 border-primary pl-3"><h2 className="text-xl font-bold uppercase tracking-tight">Admin Login</h2></div>

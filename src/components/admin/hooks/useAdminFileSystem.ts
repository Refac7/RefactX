...
export function useAdminFileSystem(
  ...
  const loadFile = async (name: string, isData = false, path?: string) => {
    ...
    try {
      ...
      if (res.status === 404 && isData) {
        editor.setFilename(name); editor.setJsonContent('[]'); editor.setParsedJson([]);
        editor.setCurrentMode('data'); editor.setEditorMode('visual'); editor.setEditingItemIndex(null); setMobileView('editor');
      ...
      if (isData) {
        editor.setFilename(name); editor.setCurrentMode('data');
        try {
          const parsed = JSON.parse(data.content);
          editor.setJsonContent(JSON.stringify(parsed, null, 2));
          editor.setParsedJson(Array.isArray(parsed) ? parsed : []);
          editor.setEditingItemIndex(null); editor.setEditorMode('visual');
        } catch(e) { editor.setJsonContent(data.content); editor.setEditorMode('raw'); }
      } else {
        const { meta: parsedMeta, body: parsedBody } = parseContent(data.content);
        editor.setFilename(name); editor.setMeta(parsedMeta); editor.setBody(parsedBody);
  ...
  const stageForWrite = () => {
    ...
    if (editor.currentMode === 'post') {
        ...
        try { JSON.parse(editor.jsonContent); } catch (e) { showToast('Invalid JSON format', 'error'); return; }
        finalFilename = DATA_FILES.find(f => f.name === editor.filename)?.path || editor.filename;
        content = editor.jsonContent;
    }

    setQueue(prev => {
        const newItem: QueueItem = { id: Date.now().toString(), type: 'write', filename: finalFilename, content, status: 'pending', isDataFile: editor.currentMode === 'data' };
  ...
  const handleNewPost = async () => {
    ...
    editor.setCurrentMode('post'); editor.setFilename(''); editor.setBody(''); editor.setMeta(DEFAULT_META);
  ...
  const loadFromQueue = (item: QueueItem) => {
    ...
    try {
        ...
        if (item.isDataFile) {
            editor.setCurrentMode('data'); editor.setJsonContent(item.content || '');
            ...
            editor.setMeta(m); editor.setBody(b);
        }
        setMobileView('editor');
  ...
  return { remoteFiles, queue, isLoadingFiles, isProcessingQueue, isFetchingContent, fetchRemoteFiles, loadFile, stageForWrite, stageForDelete, processQueue, handleNewPost, loadFromQueue, removeFromQueue };
}
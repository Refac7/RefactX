  ...
  try {
    ...
    return data.results
      .map((block: any) => {
        ...
        switch (type) {
          case 'paragraph':
            return `${parseRichText(blockData.rich_text)}\n\n`
          case 'heading_1':
            return `# ${parseRichText(blockData.rich_text)}\n\n`
          case 'heading_2':
            return `## ${parseRichText(blockData.rich_text)}\n\n`
          ...
          case 'numbered_list_item':
            return `1. ${parseRichText(blockData.rich_text)}\n`
          case 'quote':
            return `> ${parseRichText(blockData.rich_text)}\n\n`
          case 'code':
  ...
  try {
    ...
    const feed = await Promise.all(
      data.results.map(async (page: any) => {
        ...
        const exactDate = dateProp && dateProp.includes('T') ? dateProp : props.Created?.created_time || page.created_time
